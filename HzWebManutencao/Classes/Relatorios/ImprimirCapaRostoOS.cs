...
namespace HzWebManutencao
{
    public class FechamentoMes
    {
        ...
        protected static pdfTable SubCabRelatorio(pdfDocument myDoc, pdfPage myPage, int top)
        {
            ...
            myTable.tableHeader.addColumn(40, predefinedAlignment.csCenter);
            myTable.tableHeader[cl++].addText("Início");
        ...
        public static pdfDocument ImprimeCapaRostoOS(DataTable table, string DataInicial, string DataFinal)
        {
            ...
            decimal ValorBdiLucro = 0;
            decimal ValorBdiTributo = 0;

            if(string.IsNullOrEmpty(table.Rows[0]["cmpPeBDILucro"].ToString())==false)
            {
                ValorBdiLucro   = totalMat.Valor * decimal.Parse(table.Rows[0]["cmpPeBDILucro"].ToString())/100;
            }
            if(string.IsNullOrEmpty(table.Rows[0]["cmpPeBDITributos"].ToString())==false)
            {
                ValorBdiTributo = totalMat.Valor * decimal.Parse(table.Rows[0]["cmpPeBDITributos"].ToString())/100;
            }
                ...
                if (Tamanho > 0)
                {
                    ...
                    for (int Linha = 1; Linha <= QtdLin; Linha++)
                    {
                        ...
                        if (Linha == 1)
                        {
                            ...
                            myRow[cl++].addText(row["cmpDtInicioAtendimento"].ToString() == "" ? "" : row["cmpDtInicioAtendimento"].ToString().Substring(0, 10));
