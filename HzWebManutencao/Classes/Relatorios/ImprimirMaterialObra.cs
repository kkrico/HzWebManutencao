...
{
    public class ImprimirMaterialObra
    {
        #region Variables
        /// <summary>
        /// Documento PDF
        ...
        protected void CabRelatorio()
        {
            ...
            //top += 10;
            myPage.addText("LISTA DE TODOS OS MATERIAIS", 220, getTop(myPage, top), myDoc.getFontReference(strFont), 12, pdfColor.Black);
            top += 20;
        }
         ...
         protected pdfTable ItensMaterial()
        {
            ...
            myTable.tableHeader.addColumn(270, predefinedAlignment.csLeft);
            ...
            myTable.tableHeader[cl++].addText("QTD.CONT.");

            myTable.tableHeader.addColumn(50, predefinedAlignment.csRight);
            myTable.tableHeader[cl++].addText("QTD.UTIL.");

            myTable.tableHeader.addColumn(50, predefinedAlignment.csRight);
            myTable.tableHeader[cl++].addText("EST.ATUAL");
        ...
        public void ImprimeMaterial(DataTable table)
        {
            ...
            foreach (DataRow row in table.Rows)
            {
                ...
                if ((top + (QtdLin * nTamLinPixel)) > 782)
                {
                    myTable = QuebraPagina(myTable);
                }
                ...
                for (int Linha = 1; Linha <= QtdLin; Linha++)
                {
                    ...
                    if (Linha == 1)
                    {
                        ...
                        myRow[cl++].addText(row["cmpNuQtdContrato"].ToString()  == "0,00" ? "" : row["cmpNuQtdContrato"].ToString());
                        myRow[cl++].addText(row["QTUTILIZADO"].ToString()       == "0,00" ? "" : row["QTUTILIZADO"].ToString());
                        myRow[cl++].addText(row["QTDESTOQUE"].ToString()        == "0,00" ? "" : row["QTDESTOQUE"].ToString());
                    }
                    else
                    {
                        cl = 0;
